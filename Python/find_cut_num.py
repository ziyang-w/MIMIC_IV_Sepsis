...
def del_missing(missing_rate=0.1,
                file_path=r'D:\Data\secret\MIMIC_MY',
                file_name='sepsis_refine.csv',
                outName=''):
...
def Boxing_Numeric(dat_data, cut=4):
    ...
    for var in DFBoxed.columns.values:
        if type(DFBoxed[var][0]) != str:
            if DFBoxed[var].mode()[0] not in [0, 0.5, 1, 2, 3, 4, 6, 7, 8, 100,]:  # 众数不为1,0，说明为连续变量
...
if __name__ == '__main__':
    ...
    fold = 3
    # 可以优化的指标：
    features_nums=range(6,21)
    feature_cuts=range(2,8)
    ...
    for feature_cut in tqdm(feature_cuts):
        # 分箱 & 变量数值化
        ...
        for features_num in features_nums:
            ...
            hour = '%d_'%features_num
            log_path = os.path.join(filePath, 'log', fileName.split('.')[0], 'cut_%d'%feature_cut)
            ...
            #     tprs[-1][0] = 0.0
            #
            #     p, r, f1, AUC = PRF1(np.array(ytest), ypre, yprob,threshold=opt[0])
            #     cols['P'].append(p)
            #     cols['R'].append(r)
            #     cols['F1'].append(f1)
            #     cols['AUC'].append(AUC)
            ...
            #     i += 1
            #
            # plot_ROC_kfold(tprs, opts, l='Logistic Regression')
            # pd.DataFrame(cols).to_csv(os.path.join(log_path, hour) + 'result_LR.csv')
            #
            # best_param['cut'].append(feature_cut)
            # best_param['features_num'].append(features_num)
            # best_param['P_mean'].append(np.mean(cols['P']))
            # best_param['R_mean'].append(np.mean(cols['R']))
            # best_param['F1_mean'].append(np.mean(cols['F1']))
            # best_param['AUC_mean'].append(np.mean(cols['AUC']))
            # best_param['AUC_best'].append(LRbest['AUC'])
            # best_param['model'].append('LR')
            ...
            #     ytest = yDF.iloc[test_index]
            #
            #     rf = RandomForestClassifier(random_state=random_state).fit(np.array(xtrain), np.array(ytrain))
            ...
            #     i += 1
            # plot_ROC_kfold(tprs, opts, l='Random Forest')
            ...
            # best_param['F1_mean'].append(np.mean(cols['F1']))
            # best_param['AUC_mean'].append(np.mean(cols['AUC']))
            # best_param['AUC_best'].append(RFbest['AUC'])
            # best_param['model'].append('RF')
            ...
            #
            # plot_ROC_kfold(tprs, opts, l='Bayesian Network')
            ...
            # best_param['F1_mean'].append(np.mean(cols['F1']))
            # best_param['AUC_mean'].append(np.mean(cols['AUC']))
            # best_param['AUC_best'].append(BNbest['AUC'])
            # best_param['model'].append('BN')
            ...
            for train_index, test_index in tqdm(KFold):
                ...
                model = svm.SVC(kernel='rbf', gamma='auto', C=1, probability=True).fit(np.array(xtrain),
                                                                                       np.array(ytrain))
                ypre = model.predict(np.array(xtest))
            ...
            best_param['F1_mean'].append(np.mean(cols['F1']))
            best_param['AUC_mean'].append(np.mean(cols['AUC']))
            best_param['AUC_best'].append(SVMbest['AUC'])
            best_param['model'].append('SVM')
            # with open(os.path.join(log_path,hour)+'best_result_LR.json','w') as f:
            #     f.write(json.dumps(LRbest))
            ...
            # best_param['F1_mean'].append(np.mean(cols['F1']))
            # best_param['AUC_mean'].append(np.mean(cols['AUC']))
            # best_param['AUC_best'].append(XGBbest['AUC'])
            # best_param['model'].append('XGB')
            # # with open(os.path.join(log_path,hour)+'best_result_LR.json','w') as f:
            # #     f.write(json.dumps(LRbest))
            #
            # ############################################## LightGBM Model  #################################
            ...
            # best_param['F1_mean'].append(np.mean(cols['F1']))
            # best_param['AUC_mean'].append(np.mean(cols['AUC']))
            # best_param['AUC_best'].append(LGBMbest['AUC'])
            # best_param['model'].append('LGBM')
            # # with open(os.path.join(log_path,hour)+'best_result_LR.json','w') as f:
            # #     f.write(json.dumps(LRbest))
    pd.DataFrame(best_param).to_csv(os.path.join(filePath, 'log', fileName.split('.')[0],'best_param3.csv'))
